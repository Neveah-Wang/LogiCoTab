...
from lib.make_dataset import make_dataset_for_uncondition, prepare_dataloader_withoutY
...
def train(
    ...
    K = np.array(dataset.get_category_sizes('train'))   # K = [2, 9, 16, 7, 15, 6, 5, 2, 42]
    if len(K) == 0 or raw_config['Transform']['cat_encode_policy'] == 'one-hot':
        K = np.array([0])
    ...
    train_loader = prepare_dataloader_withoutY(dataset, split='train', batch_size=batch_size)
