...
# ================= 2. 数据读取 =================
datasets = ['shopper', 'churn', 'adult', 'obesity', 'page', 'buddy', 'mammography', 'yeast_me2', 'bean', 'magic']
for dataname in datasets:
    df = pd.read_csv(f'D:\Study\自学\表格数据生成\LogiCoTab-vae\exp/{dataname}\CoTable/vae_loss_detailed.csv')
    ...
    ax.set_xlabel('训练轮次', fontsize=24, fontweight='bold', labelpad=10)
    ax.set_ylabel('类原型距离', fontsize=24, fontweight='bold', labelpad=10)

    ax.grid(True, linestyle='--', alpha=0.4, which='both', linewidth=0.8)
    ...
    plt.tight_layout()
    plt.savefig(f'prototype_distance/{dataname}.pdf')
    plt.show()