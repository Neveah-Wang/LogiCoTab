...
class Transformer(nn.Module):
    def __init__(self, raw_config, device) -> None:
        ...
        self.attention = MultiHeadSelfAttention(dim_t, n_head)
        ...
        #     nn.LayerNorm(dim_t),
        #     nn.Linear(dim_t, dim_t * mult * 2),
        #     nn.SiLU(),
        #     nn.Linear(dim_t * mult * 2, dim_t * mult * 4),
        #     nn.SiLU(),
        #     nn.Linear(dim_t * mult * 4, dim_t * mult * 2),
        #     nn.SiLU(),
        #     nn.Dropout(ff_dropout),
        #     nn.Linear(dim_t * mult * 2, dim_t)
        # )
        layers = []
    ...
    def forward(self,
        ...
        x_residual = x
        x = self.norm(x)
        x = self.attention(cls_sum, x, x)
        x = x_residual + x
        x = self.feedforward(x) + x

        x_residual = x
        x = self.norm(x)
        x = self.attention(cls_label, x, x)
        x = x_residual + x
        x = self.feedforward(x) + x
